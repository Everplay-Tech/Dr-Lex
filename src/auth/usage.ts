...
export async function checkUsageLimit(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const usage = await usageDB.getUsage(user.id, today + 'T00:00:00.000Z');
    const usedToday = Array.isArray(usage) ? usage.reduce((sum: number, u: any) => sum + u.energy_used, 0) : 0;
